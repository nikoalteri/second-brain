...
**Post-review correction, round 1 (code review, 2026-09-17):** the implementation initially flagged cards using `created_at < 2026-08-06` (the bug's introduction date) as the CHECK criterion — this was still wrong, for a reason distinct from the payment-mutation framing above. A card created on 2026-09-01 (after the bug, before this fix deployed) was just as exposed to the nightly recompute as an older card, yet the bug-introduction-date threshold left it unflagged. Round-1 fix: compare against a `fixDeployedAt` constant (2026-09-17, the migration's own timestamp) instead of `bugIntroducedAt`.

**Post-review correction, round 2 (code review, 2026-09-17):** round 1's `fixDeployedAt` constant was itself unsound — a migration's filename timestamp records when the file was authored, not when it actually ran in a given environment. If a real deploy happens later than that timestamp, any card created in between would be wrongly excluded from CHECK while still having been exposed to the pre-fix nightly recompute in production. There is no reliable in-data signal for "was this card created before opening_balance existed in this specific environment" — no deploy-timestamp table, no code-version column on the card row. **Final fix: drop date-based flagging entirely.** `credit-cards:balance-audit` now lists every active card, sorted oldest-created-first (a sorting aid for manual review prioritization, not a pass/fail signal), and its message tells the user to cross-check all of them — conservative by construction, since it cannot silently exclude a genuinely at-risk card.
