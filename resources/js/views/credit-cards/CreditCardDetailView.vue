    ...
    <AppLayout>
        ...
        <template v-else-if="card">
            <div class="mb-6 flex items-center justify-between">
                <div>
                    <h1 class="text-xl font-semibold text-white">{{ card.name }}</h1>
                    <p class="mt-1 text-sm text-gray-400">{{ card.type }} card · {{ card.interest_rate }}% interest</p>
                </div>
                <router-link
            ...
            <div class="mb-6 rounded-xl border border-gray-700 bg-gray-800 p-6">
                <div class="mb-4 grid grid-cols-2 gap-6 md:grid-cols-3">
                    ...
                    <div>
                        ...
                        <p class="mt-1 font-mono text-xl font-semibold text-white">{{ formatCurrency(card.credit_limit ?? 0) }}</p>
                    </div>
                    <div>
            ...
            <h2 class="mb-4 text-xl font-semibold text-white">Billing Cycles</h2>
            ...
            <div class="flex flex-col gap-3">
                ...
                >
                    <button
                        class="flex w-full items-center justify-between p-4 transition-colors hover:bg-gray-700/40"
                        @click="toggleCycle(cycle.id)"
                    >
                        <div class="flex min-w-0 items-center gap-4">
                            <div class="min-w-0 text-left">
                                <p class="text-sm font-normal text-white">{{ cycle.period_month ?? cycle.period_start_date }}</p>
                                <p class="mt-0.5 text-sm text-gray-400">Due: {{ cycle.due_date ?? '—' }}</p>
                            </div>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <component :is="expandedCycles.has(cycle.id) ? ChevronUpIcon : ChevronDownIcon" class="h-4 w-4 shrink-0 text-gray-400" />
                        </div>
                    </button>

                    <div v-if="expandedCycles.has(cycle.id)" class="border-t border-gray-700 px-4 py-3">
                        ...
                        <div v-else class="divide-y divide-gray-700/50">
                            ...
                            >
                                <div>
                                    <p class="text-sm text-gray-100">{{ expense.description ?? 'Expense' }}</p>
